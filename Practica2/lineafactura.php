...
$resultado=mysql_query("insert detalle_factura(num_factura,unidades,referencia) values ('$numfactura','$unidades','$referencia')");
if($resultado==0)
{
echo " <h4>Error al introducir la linea de detalle en la Base de Datos.</h4><br>";
}
else
{
echo "<h4>La linea de detalle se ha introducido con &eacute;xito en la Base de Datos.</h4><br>";
}
...
echo 		
			...
			<input type='hidden' name='numfactura' value='$numfactura' required/><br>
...
$resultado=mysql_query("select referencia,importe,porcentaje from repuestos");
if($resultado==0)
{
...
//Tenemos que hacer un update a la factura para añadirle el coste de los repuestos tendremos que coger el importe y el porcentaje de ganancia 
//si importe=100 porcentaje=10 tendremos que cobrarle 110 al cliente por cada unidad.

$cobrorepuesto=$importe*(($porcentaje/100)+1);
$baseimponible=$baseimponible+$cobrorepuesto*$unidades;
$total=$baseimponible *(($iva/100)+1);
$resultado=mysql_query("update factura set base_imponible='$baseimponible',total='$total' where num_factura='$numfactura'");
if($resultado==0)
{
echo " <h4>Error al introducir la linea en la Base de Datos.</h4><br>";
}
//Si funciona pasamos tambien la baseimponible por si queremos añadir otra linea de factura
else
{
	
}
echo "<input type='hidden' name='baseimponible' value='$baseimponible' required/><br>";
?>
					
		<center>
		<input type=submit value="Añadir otra linea a la factura">
